...
sleep 1
./client < ../$REL_TEST_DIR/test01gen.dsl  > $STUDENT_OUTPUT_DIR/test01gen.client.debug.out
echo ""
echo "running test 2"
./server > $STUDENT_OUTPUT_DIR/test02gen.server.debug.out &
sleep 1
./client < ../$REL_TEST_DIR/test02gen.dsl > $STUDENT_OUTPUT_DIR/test02gen.client.debug.out
